...
class Modelgenerator():
    def __init__(self, dbsrc, dbmgr, dbtag, tables_in=None):
        ...
        # 表对象文件处理
        self.filedir_tab = mdtool.Variable.TMP_PATH + os.sep + 'tables_generator' + os.sep + '%s2%s' % (
        ...
        if not os.path.exists(self.filedir_tab):
            ...
            file.close()
        # 约束对象文件处理
        self.filedir_cst = mdtool.Variable.TMP_PATH + os.sep + 'constraints_generator' + os.sep + '%s2%s' % (
            self.dbtype, self.dbtype_tag)
        if not os.path.exists(self.filedir_cst):
            ...
            file.close()
        # 索引对象文件处理
        self.filedir_idx = mdtool.Variable.TMP_PATH + os.sep + 'indexes_generator' + os.sep + '%s2%s' % (
            self.dbtype, self.dbtype_tag)
        ...
        for tabname in tabnames:
            ...
            if self.dbtype == 'oracle':
                if self.dbtype_tag == 'mysql':
                    ...
                    with open(self.filedir_tab + os.sep + '%s2%s_model.sql' % (self.dbtype, self.dbtype_tag), 'a') as f:
                    ...
                    mdtool.log.info(
                        "%s2%s----------------------------%s表模型开始构建----------------------------" % (
                            self.dbtype, self.dbtype_tag, tabname))
                    for elem in result:
                        table_comment = elem[0]
                        column_name = elem[1]
...
if __name__ == '__main__':
    ...
    dbtag = mdtool.xmler('MYSQL_172.21.86.205').dbCFGInfo()
